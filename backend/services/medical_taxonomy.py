...
class MedicalTermExtractor:
    ...
    def __init__(self):
        ...
        self.medical_terms_dict = {
            '心血管': ['心脏病', '高血压', '冠心病', '心律不齐', '心肌梗死'],
            '呼吸系统': ['肺炎', '哮喘', '慢阻肺', '肺结核', '感冒'],
            '消化系统': ['胃炎', '胃溃疡', '肝炎', '胆结石', '肠炎'],
            '神经系统': ['头痛', '癫痫', '中风', '帕金森', '老年痴呆'],
            '内分泌': ['糖尿病', '甲亢', '甲减', '肥胖症'],
            '感染性疾病': ['发热', '感染', '病毒', '细菌', '真菌']
        }
    ...
    def extract_medical_terms(self, text: str) -> List[str]:
        ...
        # 基于词典匹配
        for category, term_list in self.medical_terms_dict.items():
...
class MedicalDocumentClassifier:
    ...
    def __init__(self):
        ...
        self.department_keywords = {
            MedicalDepartment.INTERNAL_MEDICINE: [
                '内科', '心血管', '呼吸', '消化', '内分泌', '肾内科', '血液科',
                '心脏病', '高血压', '糖尿病', '肺炎', '哮喘', '胃炎', '肝炎'
            ],
            MedicalDepartment.SURGERY: [
                '外科', '手术', '骨科', '神经外科', '胸外科', '普外科',
                '骨折', '肿瘤切除', '阑尾炎', '胆囊炎'
            ],
            MedicalDepartment.SPECIALIZED: [
                '眼科', '耳鼻喉', '皮肤科', '泌尿科', '口腔科', '康复科',
                '白内障', '中耳炎', '湿疹', '前列腺', '牙科'
            ],
            ...
            MedicalDepartment.OBSTETRICS_GYNECOLOGY: [
                '妇产科', '妇科', '产科', '妊娠', '分娩', '月经',
                '子宫', '卵巢', '乳腺', '孕妇'
            ],
        ...
        self.document_type_keywords = {
            ...
            DocumentType.DRUG_REFERENCE: [
                '药物', '用药', '说明书', '处方', '剂量'
            ],
            DocumentType.PROCEDURE_GUIDE: [
                '操作', '手术', '检查', '检验', '技术'
            ],
            DocumentType.GENERAL_REFERENCE: [
                '病例', '研究', '教材', '手册', '教育'
            ]
        }
    ...
    def _classify_disease_categories(self, text: str) -> List[DiseaseCategory]:
        ...
        # 基于关键词匹配
        disease_keywords = {
            DiseaseCategory.CARDIOVASCULAR: ['心脏', '心血管', '高血压', '冠心病'],
            DiseaseCategory.RESPIRATORY: ['肺', '呼吸', '哮喘', '肺炎'],
            DiseaseCategory.DIGESTIVE: ['胃', '肠', '肝', '消化'],
            DiseaseCategory.NEUROLOGICAL: ['神经', '大脑', '头痛', '癫痫'],
            DiseaseCategory.MENTAL_DISORDERS: ['精神', '心理', '抑郁', '焦虑'],
            DiseaseCategory.INFECTIOUS: ['感染', '病毒', '细菌', '发热'],
            DiseaseCategory.CHRONIC_DISEASES: ['慢性', '糖尿病', '肿瘤', '癌症'],
            DiseaseCategory.GENERAL_CONDITIONS: ['皮肤', '泌尿', '妊娠', '外伤']
        }
    ...
    def _calculate_confidence(self, metadata: MedicalMetadata, text: str) -> float:
        ...
        if metadata.medical_terms:
            confidence += len(metadata.medical_terms) * 0.1

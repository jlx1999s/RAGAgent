...
class QwenMedicalIntentRecognizer:
    ...
    def create_intent_prompt(self, query: str) -> str:
        """创建意图识别的提示词（包含是否为医疗问题与候选项）"""
        prompt = f"""你是一个专业的医疗意图识别专家。请先判断该问题是否属于医疗健康领域，然后识别最相关的科室、疾病类别和文档类型，并给出候选项。
...
{{
    ...
    "candidates": {{
        "departments": ["候选科室1", "候选科室2"],
        "disease_categories": ["候选疾病类别1", "候选疾病类别2"],
        "document_types": ["候选文档类型1", "候选文档类型2"]
    }},
    "confidence": 0.95,
    "reasoning": "简要说明判断依据与选择原因",
    "keywords": ["从问题中提取的关键医疗词汇"]
}}

规则：
1. is_medical 为布尔值；若不是医疗问题，请置为 false，并将 department/disease_category/document_type 设为 null。
2. 所有分类和候选项必须从给定可选项中选择；每个候选列表最多列出 3 项。
3. confidence 应为 0-1 之间的数值。
4. reasoning 简要说明分析过程与选择依据。
5. 输出必须为纯 JSON，不要包含多余文本。
"""
        return prompt

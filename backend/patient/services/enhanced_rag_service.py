...
class EnhancedMedicalRAGService:
    ...
    async def medical_answer_stream(
        ...
        # 构建体检报告部分
        medical_report_section = ""
        if medical_report and medical_report.strip():
            report_type_text = f"（{report_type}）" if report_type else ""
            medical_report_section = f"患者体检报告{report_type_text}：\n{medical_report}\n\n"
        ...
        if context_text and context_text != "(no medical documents found)":
            ...
            # 即使没有医疗文档上下文，也可能有体检报告
            if medical_report_section:
                user_prompt = (
                    f"基于以下体检报告回答用户问题：\n\n"
                    f"问题：{question}\n\n"
                    f"{medical_report_section}"
                    f"回答要求：\n"
                    f"1. 回答要简洁明了，控制在200字以内\n"
                    f"2. 使用通俗易懂的语言，避免过多专业术语\n"
                    f"3. 重点突出最关键的建议\n"
                    f"4. 简单提醒咨询医生即可\n"
                    f"5. 不需要复杂的格式，直接回答\n"
                    f"6. 请结合体检报告内容给出针对性建议\n"
                )
                has_context = True
            else:
...
async def medical_answer_stream(
    ...
    async for item in enhanced_rag_service.medical_answer_stream(
        question, citations, context_text, metadata, session_id, enable_safety_check, medical_report, report_type
    ):
        yield item